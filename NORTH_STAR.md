...
- [x] Ambassador/comp access: a `comp_access` flag on `shiftwell_profiles` (only the service role can set it) gives full access whatever Stripe says, and the Stripe webhook skips those users. Set for Jonny and Ashleigh. *(24 Sept 2026, 8b77a56. Ashleigh's Stripe subscription was cancelled; her row and access are unchanged. The webhook skip will be proven live once the server key is fixed.)*
